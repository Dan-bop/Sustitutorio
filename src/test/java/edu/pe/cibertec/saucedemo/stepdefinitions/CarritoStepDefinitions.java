...
public class CarritoStepDefinitions {
    ...
    public void sheAddsProductToCart(String name) {
        OnStage.theActorInTheSpotlight().attemptsTo(AgregarAlCarrito.producto(name));
    }

    @And("she removes the product {string} from the cart")
    public void sheRemovesProductFromCart(String name) {
        OnStage.theActorInTheSpotlight().attemptsTo(EliminarCarrito.producto(name));
    }
    ...
    public void sheShouldSeeTheProductInTheCart(String name) {
        OnStage.theActorInTheSpotlight().should(seeThat(TheCartContents.displayed(), hasItem(name)));
    }

    @Then("the cart icon should display {string}")
    public void cartIconShouldDisplay(String count) {
        OnStage.theActorInTheSpotlight().should(seeThat(TheCartBadge.displayed(), equalTo(count)));
    }

    @Then("the cart should contain {string} and {string}")
    public void cartShouldContain(String item1, String item2) {
        OnStage.theActorInTheSpotlight().attemptsTo(Click.on(Target.the("cart icon").locatedBy(CarritoPage.CART_ICON)));
        OnStage.theActorInTheSpotlight().should(seeThat(TheCartContents.displayed(), hasItems(item1, item2)));
    }

    @Then("the cart should only contain {string}")
    public void cartShouldOnlyContain(String item) {
        OnStage.theActorInTheSpotlight().attemptsTo(Click.on(Target.the("cart icon").locatedBy(CarritoPage.CART_ICON)));
        OnStage.theActorInTheSpotlight().should(seeThat(TheCartContents.displayed(), hasItem(item)));
        OnStage.theActorInTheSpotlight().should(seeThat(TheCartContents.displayed(), hasSize(1)));
    }
}